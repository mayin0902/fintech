...
class MoEBlockOptimized(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.shared_chunk_size = 262144
        self.expert_chunk_size = 262144
        self.use_checkpoint = False
        self.long_threshold = 327680
        self.long_shared_chunk_size = 131072
        self.long_expert_chunk_size = 262144
        self.long_use_checkpoint = False
        self._compile_local_methods({"shared", "post"})

    def _compile_local_methods(self, targets):
        if os.getenv("MOE_DISABLE_COMPILE", "0") == "1":
            return
        kwargs = {
            "dynamic": os.getenv("MOE_COMPILE_DYNAMIC", "1") != "0",
            "fullgraph": os.getenv("MOE_COMPILE_FULLGRAPH", "0") == "1",
        }
        torch_mode = os.getenv("MOE_COMPILE_TORCH_MODE", "reduce-overhead").strip()
        if torch_mode:
            kwargs["mode"] = torch_mode
        if "route" in targets:
            self._route = torch.compile(self._route, **kwargs)
        if "shared" in targets:
            self._shared_chunk = torch.compile(self._shared_chunk, **kwargs)
        if "post" in targets:
            self._post_norm = torch.compile(self._post_norm, **kwargs)
    ...
    def _checkpoint_if(self, use_checkpoint, fn, *args):
        if not self.training or not use_checkpoint:
            return fn(*args)
        if not any(torch.is_tensor(arg) and arg.requires_grad for arg in args):
            return fn(*args)
        return checkpoint(
    ...
    def _is_long_input(self, num_tokens):
        return num_tokens >= self.long_threshold

    def _shared_limit(self, num_tokens):
        if self._is_long_input(num_tokens):
            return self.long_shared_chunk_size
        return self.shared_chunk_size

    def _expert_limit(self, num_tokens):
        if self._is_long_input(num_tokens):
            return self.long_expert_chunk_size
        return self.expert_chunk_size

    def _checkpoint_enabled_for(self, num_tokens):
        if self._is_long_input(num_tokens):
            return self.long_use_checkpoint
    ...
    def _shared_forward(self, hidden_flat):
        ...
        chunk_size = self._shared_limit(num_tokens)
        use_checkpoint = self._checkpoint_enabled_for(num_tokens)
        if num_tokens <= chunk_size:
            return self._checkpoint_if(use_checkpoint, self._shared_chunk, hidden_flat)

        shared_output = torch.empty_like(hidden_flat)
        for start in range(0, num_tokens, chunk_size):
            end = min(start + chunk_size, num_tokens)
            shared_output[start:end] = self._checkpoint_if(
                use_checkpoint,
                self._shared_chunk,
    ...
    def _shared_forward_add_(self, hidden_flat, target):
        num_tokens = hidden_flat.shape[0]
        chunk_size = self._shared_limit(num_tokens)
        use_checkpoint = self._checkpoint_enabled_for(num_tokens)
        if num_tokens <= chunk_size:
            target.add_(self._checkpoint_if(use_checkpoint, self._shared_chunk, hidden_flat))
            return target

        for start in range(0, num_tokens, chunk_size):
            end = min(start + chunk_size, num_tokens)
            target[start:end].add_(
                self._checkpoint_if(
                    use_checkpoint,
                    self._shared_chunk,
                    hidden_flat[start:end],
    ...
    def _experts_forward(self, hidden_flat, selected_experts, top_k_weights):
        ...
        num_tokens = hidden_flat.shape[0]
        chunk_size = self._expert_limit(num_tokens)
        use_checkpoint = self._checkpoint_enabled_for(num_tokens)
        ...
        for expert_idx in expert_hit.tolist():
            ...
            for start in range(0, token_idx.numel(), chunk_size):
                ...
                current_hidden_states = self._checkpoint_if(
                    use_checkpoint,
                    self._expert_chunk,
                    current_state,
                    expert_idx,
    ...
    def forward(self, hidden_states):
        ...
        self._shared_forward_add_(hidden_flat, routed_output)
        output = self._post_norm(routed_output)
